...
export default function Feed() {
  ...
  function handleTypeChange(nextType) {
    setActiveType(current => (current === nextType ? 'all' : nextType))
    setTimeout(() => {
  ...
  if (featuredQuestion && !allQuestions.find(question => question.id === featuredQuestion.id)) {
    allQuestions.unshift(featuredQuestion)
  }
      ...
      <div className="feed-shell">
        <aside className="feed-sidebar">
          ...
          <div className="sidebar-actions">
            ...
            <button type="button" className="sidebar-secondary-action">
              Settings
            </button>
            <button type="button" className="sidebar-secondary-action">
              Support
        ...
        <div className="feed-main-column">
          <header className="feed-topbar">
            <div className="feed-tabs" role="tablist" aria-label="Feed lanes">
              {TOP_TABS.map(tab => {
                const isActive = activeType === tab.key || (tab.key === 'all' && activeType === 'all')
                return (
                  <button
                    key={tab.key}
                    type="button"
              ...
              <>
                ...
                <section className="feed-section" ref={contentRef}>
                  <div className="feed-section-heading">
                    <div>
                      <p className="feed-section-kicker">{activeCategory === 'All' ? 'Across the feed' : activeCategory}</p>
                      <h3>{visibleTypeLabel}</h3>
                    </div>
                    <p className="feed-section-meta">
                      {activeQuestionCount} active questions
                    </p>
                  </div>
                ...
                <section className="feed-metrics">
                  <MetricCard
                    ...
                    value={formatCount(activeQuestionCount)}
                    body="Questions currently visible in this filtered view."
                  />
                  <MetricCard
                    label="Captured Votes"
                    value={formatCount(allVisibleVotes)}
                    body="Anonymous and verified responses already shaping the signal."
                  />
                  <MetricCard
  ...
  return (
    <section className="featured-card" style={{ '--featured-accent': accent }}>
      <div className="featured-copy">
        ...
        <h3>{type === 'statement' ? `"${question.text}"` : question.text}</h3>
        ...
        <div className="featured-actions">
          ...
          <div className="featured-stats">
            ...
            {verifiedVotes > 0 && <strong>{formatCount(verifiedVotes)} verified</strong>}
          </div>
        </div>
      </div>
      ...
      <button type="button" className="featured-media-shell" onClick={onOpen}>
        {question.image_url ? (
          <img src={question.image_url} alt="" className="featured-media" />
        ) : (
    ...
    <article className="feed-card" style={{ '--card-accent': accent }}>
      <button type="button" className="feed-card-button" onClick={onOpen}>
        ...
        <div className="feed-card-media-shell">
          {question.image_url ? (
            <img src={question.image_url} alt="" className="feed-card-media" />
          ) : (
        ...
        <div className="feed-card-body">
          <h4>{type === 'statement' ? `"${question.text}"` : question.text}</h4>
          <p>{getQuestionSummary(question, counts)}</p>
        </div>
